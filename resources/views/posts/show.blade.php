    ...
    <header class="surface-light border-b border-rule-light pb-10 pt-32 sm:pb-14 sm:pt-40">
        <div class="site-container">
            <div class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end">
                <div class="min-w-0">
                    <a class="font-data text-xs uppercase tracking-[0.18em] text-ink-muted underline-offset-4 hover:text-ink hover:underline" href="{{ route('posts.index') }}">{{ trans('page.technical.archive') }}</a>
        ...
        <div class="site-container grid gap-12 pt-12 lg:grid-cols-[minmax(0,1fr)_17rem] lg:gap-16">
                ...
                <section class="mt-14 grid gap-7 border-y border-rule-light py-8 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center" aria-labelledby="author-title">
                    <div class="flex items-start gap-5">
                        ...
                        <div>
                            ...
                            <p class="mt-2 max-w-xl whitespace-normal text-ink-muted">{{ trans('page.me_short') }}</p>
                        </div>
                    </div>
                    <a class="catalog-action catalog-action--quiet-light" href="mailto:{{ config('blog.mail') }}">{{ trans('page.contact_me') }}</a>
                </section>

                @if ($related->isNotEmpty())
                    <section class="mt-14" aria-labelledby="related-title">
                        <h2 id="related-title" class="text-4xl text-ink">{{ trans('page.related') }}</h2>
            ...
            <aside class="space-y-8 lg:sticky lg:top-28 lg:self-start">
                <section class="border-t border-rule-light pt-4" aria-labelledby="share-title">
                    <h2 id="share-title" class="font-data text-xs uppercase tracking-[0.18em] text-ink-muted">{{ trans('page.share') }}</h2>
                    <p class="mt-3 whitespace-normal text-sm text-ink-muted">{{ trans('page.share_text') }}</p>
