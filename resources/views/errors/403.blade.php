...
@section('content')
    <section class="error-page flex min-h-screen items-center border-b border-rule-dark pb-16 pt-32 text-light">
        <div class="site-container grid gap-10 md:grid-cols-[minmax(0,0.75fr)_minmax(18rem,1.25fr)] md:items-center">
            ...
            <div>
                <p class="font-data text-xs uppercase tracking-[0.2em] text-track-yellow">{{ trans('page.technical.blocked') }}</p>
                <h1 class="mt-4 text-5xl text-light sm:text-7xl">{{ trans('page.403') }}</h1>
                <a class="catalog-action catalog-action--primary mt-8" href="{{ url()->previous() }}">{{ trans('page.back') }}</a>
            </div>
        </div>
    </section>
